...
from app.utils.csv_logger import cleanup_all_logs_async
import asyncio
import logging
...
def send_sms_task(self, sms_data: dict) -> dict:
    """
    Async task to send SMS with improved error handling
    """
    try:
        ...
        try:
            result = loop.run_until_complete(sms_service.send_sms(sms_request))
...
def cleanup_logs_task(self):
    ...
    try:
        ...
        try:
            loop.run_until_complete(cleanup_all_logs_async())
